...
description: Commit and push the current Android ticket's work after showing the exact commit message, file list, push target and the code review's findings, then offer the draft pull request. Use when the user asks to commit and push their ticket, or as stage 09 of the Android workflow. Requires a passing security gate for the current changes.
---
...
2. **Require a current security pass.** Recompute the diff fingerprint; if the ticket file has no
   ...
   clears. The `review:` block is read, never required: the review is advisory.
3. **Build the message** `<KEY>: <name>` from the ticket file. Missing name → ask.
4. **Gate 3.** Show the message, the file list, the target `<branch> → <remote>`, every waiver —
   security and related-ticket — quoted, and the review summary from the ticket file, marked stale
   when its fingerprint is not the current one.
